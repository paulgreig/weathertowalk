...
- **Android APK builds** — use the **Dev Container** (`.devcontainer/`) for JDK 17, Android SDK API 34, **NDK 21.4** (gomobile rejects NDK 26+ on many hosts), and `gomobile` on `PATH`. Then `make android-apk` from the repo root. For a bare Linux host without Android Studio, `scripts/setup-android-sdk.sh` installs the CLI SDK; set `ANDROID_HOME` / `ANDROID_NDK_HOME` as printed. Demo: `docs/weathertowalk-apk-demo.mp4`, notes in `docs/APK_BUILD_VERIFICATION.md`.
